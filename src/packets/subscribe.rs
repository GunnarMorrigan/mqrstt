...
impl MqttRead for SubscriptionOptions{
    fn read(buf: &mut bytes::Bytes) -> Result<Self, DeserializeError> {
        ...
        let options = SubscriptionOptions::from_bits(buf.get_u8()).ok_or(DeserializeError::MalformedPacket)?;
        ...
        if options.contains(SubscriptionOptions::RETAIN_HANDLING1 | SubscriptionOptions::RETAIN_HANDLING2){
...
impl From<String> for Subscription{
    fn from(value: String) -> Self {
        Self(
            vec![(value, SubscriptionOptions::default())]
        )
    }
}
...
impl From<&str> for Subscription{
    fn from(value: &str) -> Self {
        Self(
            vec![(value.to_string(), SubscriptionOptions::default())]
        )
    }
}
...
impl From<&[&str]> for Subscription{
    fn from(value: &[&str]) -> Self {
        Self(
            value.iter().map(|topic| (topic.to_string(), SubscriptionOptions::default())).collect(),
        )
    }
}
...
mod tests{
    ...
    use super::{SubscriptionOptions, Subscribe, SubscribeProperties};
