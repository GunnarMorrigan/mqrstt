use std::{hint::black_box, io::Write, net::SocketAddr, sync::Arc, time::Duration};

use bytes::BytesMut;
use criterion::{criterion_group, Criterion};
use mqrstt::{ConnectOptions, NetworkBuilder, NetworkStatus};
...
fn tokio_concurrent_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("tokio_bench_concurrent_read_write_and_handling_NOP", |b| {
        ...
        b.to_async(runtime).iter_with_setup(tokio_setup, |(tcp_stream, server, addr)| async move {
